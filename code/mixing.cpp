...
// x = a*x
void sscal(vector<double>& x, double a)
{
...
// dot product x^Ty
double sdot(const vector<double>& x, const vector<double>& y)
{
        double acc = 0;
        size_t vSize = x.size();
...
// 2-norm of a vector
double snrm2(const vector<double>& v)
{
...
{   
    ...
    if( argc != 5 ) {
        cout << "usage: " << argv[0] << " <wcsp-input>  " << " 1 - Dualization method, 2 - BCD method " << " rank : -1 -> sqrt(2n), k " << "nb of roundings\n";
        return 1;
    }
    ...
    if(stoi(argv[2]) == 1)
    {
            ...
            if(debug){double rho = w.penaltyCoeff();
